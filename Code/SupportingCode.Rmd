...
$$
\tiny \frac{\partial P^{*}_{N}}{\partial a_{R}} = (r_{P} r_{R} (a_{P} r_{H} \alpha \beta (-1 + \delta) (\pi - \tau) + a_{H} q_{S} (\beta + \alpha (-1 + \delta) - \beta \tau)))/(a_{P} (a_{R} r_{P} \alpha (-1 + \delta) + a_{H} r_{R} \beta (-1 + \tau))^2)
$$

and with respect to _a~R~_,

$$
\tiny \frac{\partial P^{*}_{N}}{\partial r_{R}} = (a_{R} r_{P} (a_{H} q_{S} (\alpha - \alpha \delta + \beta (-1 + \tau)) - a_{P} r_{H} \alpha \beta (-1 + \delta) (\pi - \tau)))/(a_{P} (a_{R} r_{P} \alpha (-1 + \delta) + a_{H} r_{R} \beta (-1 + \tau))^2)
$$
...
that the formulae for $\partial PP$ vary based on whether we are calculating it 
...
$$
\tiny \frac{\partial \Phi^*_P}{\partial a_H} = \frac{r_{R} (a_{R} r_{P} r_{R} (-2 a_{H} q_{S} \alpha (-1 + \delta) + a_{P} r_{H} \beta (\alpha - \alpha \delta + \beta (-1 + \tau)) (\pi - \tau)) + (a_{R})^2 q_{S} (r_{P})^2 (-2 \alpha (-1 + \delta) + \beta (-1 + \tau)) - (a_{H})^2 q_{S} (r_{R})^2 \beta (-1 + \tau))}{a_{P} a_{R} (a_{R} r_{P} \alpha (-1 + \delta) + a_{H} r_{R} \beta (-1 + \tau))^2}
$$
...
$$
\tiny \frac{\partial \Phi^*_{P}}{\partial a_{R}} = \frac{(r_{R} (a_{H}^2 r_{R} (2 a_{R} q_{S} r_{P} \alpha (-1 + \delta) + a_{P} r_{H} r_{R} \beta^2 (\pi - \tau) (-1 + \tau)) + a_{P} a_{R}^2 r_{H} r_{P}^2 \alpha \beta (-1 + \delta) (\pi - \tau) + a_{H}^3 q_{S} r_{R}^2 \beta (-1 + \tau) + a_{H} a_{R} r_{P} (2 a_{P} r_{H} r_{R} \alpha \beta (-1 + \delta) (\pi - \tau) + a_{R} q_{S} r_{P} (\beta + 2 \alpha (-1 + \delta) - \beta \tau))))}{(a_{P} a_{R}^2 (a_{R} r_{P} \alpha (-1 + \delta) + a_{H} r_{R} \beta (-1 + \tau))^2)}
$$
...
$$
\tiny NECB^* = \frac{a_R^2 q_S r_P^2 \alpha (-1 + \delta) + a_R r_P r_R (-a_P r_H \alpha \beta (-1 + \delta) + a_H q_S (\alpha (-2 + \delta) + \beta (-2 + \tau))) + a_H r_R^2 (a_P r_H \beta (\beta - \pi (\alpha + \beta) + \alpha \tau) - a_H q_S (\alpha + 2 \beta - \beta \tau))}{a_P a_R (a_R r_P \alpha (-1 + \delta) + a_H r_R \beta (-1 + \tau))}
$$
...
$$
\small \frac{\partial NECB^*}{\partial a_{H}} = \frac{r_{P} r_{P} (2 a_{H} a_{{p}} q_{S} r_{H} \alpha^2 (-1 + \delta) + a^2_{P} (-1 + \pi) r^2_{H} \alpha^2 \beta (-1 + \delta) + a^2_{H} q^2_{S} (\beta - \pi \beta + \alpha \delta))}{a^2_{H} (a_{H} q_{S} + a_{P} (-1 + \pi) r_{H} \beta)^2}
$$

Similarly, the equilibrium partial derivative of NECB with respect to _r~H~_ is,

$$
\small\frac{\partial NECB^*}{\partial r_{H}} = -\frac{r_{P} (2 a_{H} a_{P} q_{S} r_{H} \alpha^2 (-1 + \delta) + a^2_{P} (-1 + \pi) r^2_{H} \alpha^2 \beta (-1 + \delta) + a^2_{H} q^2_{S} (\beta - \pi \beta + \alpha \delta)}{a_{H} (a_{H} q_{S} + a_{P} (-1 + \pi) r_{H} \beta)^2}
$$
...
$$
\tiny \frac{\partial NECB^*}{\partial a_{H}} = \frac{r_R (a_R^2 q_S r_P^2 \alpha (-\beta + \alpha (-2 + \delta)) (-1 + \delta) + a_H^2 q_S r_R^2 \beta (-\alpha + \beta (-2 + \tau)) (-1 + \tau) - a_R r_P r_R \alpha (-1 + \delta) (a_P r_H \beta (\alpha + \beta) (\pi - \tau) + 2 a_H q_S (\alpha + 2 \beta - \beta \tau)))}{a_P a_R (a_R r_P \alpha (-1 + \delta) + a_H r_R \beta (-1 + \tau))^2}
$$
...
The data in \@ref(fig:vd-scenario-2-plant-biomass-pd-ah-sp) are quite spread out,
so the next graph zooms in on the region around the `y = 0` line. Note that we 
removed the trend line in this next figure because using function 
`coord_cartesian()` simply zooms the plot but does not change the data 
underlying it, which causes some graphical artefacts in the trend line.
...
The graphs below (Figures \@ref(fig:vd-gsa-no-herbivore-model-plot), \@ref(fig:vd-gsa-herbivore-model-plot), and \@ref(fig:vd-gsa-predator-model-plot)) 
...
re-works of Figures \@ref(fig:vd-gsa-no-herbivore-model-plot),  \@ref(fig:vd-gsa-herbivore-model-plot), and \@ref(fig:vd-gsa-predator-model-plot), 
...
PPbp_pub <- PPbp + scale_y_continuous(
    trans = "log10",
    breaks = function(x) {
      brks <- scales::extended_breaks(Q = c(0.01, 100))(log10(x))
      10^(brks[brks %% 1 == 0])
    },
    labels = scales::math_format(format = log10)
  ) 
...
rate (left) and with herbivore recycling rate (right). Insets in each 
panel zoom in to show the area around `y = 0` in greater detail.

```{r pf-pp-summary-pd-insets, echo=TRUE, tidy=TRUE, message=FALSE, layout="l-body-outset", fig.width=10, fig.height=5, fig.cap = "Change in equilibrium primary productivity with herbivores' uptake (a~H~, **left**) and recycling (r~H~, **right**) rates. Data points in blue (red) identify instances in which---contrary to expectations---primary productivity increases (decreases) with respect to increasing values of uptake (recycling) rate. Insets show the region around _y = 0_ (dashed black line) for each panel in greater detail. Note the different y-axis scales."}
dPPahplot <- dPPah %>%
  filter(., Nutrient == "Carbon") %>%
  ggplot(., aes(x = ah, y = dPP)) +
